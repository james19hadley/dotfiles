...
assert.equal(model.formatFocusedTime(0), '0m')
assert.equal(model.formatFocusedTime(20), '20m')
assert.equal(model.formatFocusedTime(60), '1h')
assert.equal(model.formatFocusedTime(85), '1h 25m')
assert.equal(model.formatFocusedTime(120), '2h')
assert.ok(model.glyphFor('work').length > 0)
// ---- interactive manual count adjustments ---------------------------------

let adjState = model.idleState()
adjState = model.adjustTodayCount(adjState, 1, 20, T0)
assert.equal(adjState.todayCount, 1)
assert.equal(adjState.todayMinutes, 20)
adjState = model.adjustTodayCount(adjState, 2, 20, T0)
assert.equal(adjState.todayCount, 3)
assert.equal(adjState.todayMinutes, 60)
adjState = model.adjustTodayCount(adjState, -1, 20, T0)
assert.equal(adjState.todayCount, 2)
assert.equal(adjState.todayMinutes, 40)
adjState = model.setTodayCount(adjState, 0, 0, T0)
assert.equal(adjState.todayCount, 0)
assert.equal(adjState.todayMinutes, 0)
